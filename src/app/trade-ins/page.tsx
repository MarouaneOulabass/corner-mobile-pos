  ...
  return (
    <div className="p-4 pb-24">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Rachats</h1>
        <button
      ...
      <div className="mb-3">
        <input
          ...
          className="w-full px-3 py-2 border border-gray-200 dark:border-slate-600 rounded-xl text-sm bg-white dark:bg-slate-800"
        />
      </div>
      ...
      {!loading && (
        <div className="flex gap-2 mb-4">
          <div className="flex-1 bg-[#2AA8DC]/10 rounded-xl p-3 text-center">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Rachats</p>
          </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {tradeIns.map((tradeIn) => {
            ...
            return (
              <div key={tradeIn.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 overflow-hidden">
                <button
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`px-2 py-0.5 text-xs text-white rounded-full ${tradeInStatusColors[tradeIn.status] || 'bg-gray-50 dark:bg-slate-9000'}`}>
                    ...
                    <div className="text-right flex items-center gap-2 ml-2">
                      <span className="text-sm font-bold text-gray-900 dark:text-white">{formatPrice(tradeIn.offered_price)}</span>
                      <svg
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      {tradeIn.imei && (
                        <div>
                          ...
                          <p className="text-gray-700 dark:text-gray-200 font-mono text-xs">{tradeIn.imei}</p>
                        </div>
                      )}
                      {tradeIn.color && (
                        <div>
                          <p className="text-xs text-gray-400">Couleur</p>
                          <p className="text-gray-700 dark:text-gray-200">{tradeIn.color}</p>
                        </div>
                      )}
                      {tradeIn.ai_suggested_price != null && (
                        <div>
                          <p className="text-xs text-gray-400">Prix suggere IA</p>
                          <p className="text-gray-700 dark:text-gray-200">{formatPrice(tradeIn.ai_suggested_price)}</p>
                        </div>
                      )}
                      {tradeIn.processor?.name && (
                        <div>
                          <p className="text-xs text-gray-400">Traite par</p>
                          <p className="text-gray-700 dark:text-gray-200">{tradeIn.processor.name}</p>
                        </div>
                      )}
                    </div>
